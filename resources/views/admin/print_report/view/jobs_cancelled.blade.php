                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        <div class="card-title">
                            <h3 class="card-label">Jobs cancelled</h3>
                        </div>
                        <div class="card-toolbar">
                            <a href="{{route($route_name.'.reportPrintActionPrint',['id' => $user_id.'?action=jobs_cancelled'])}}" target="_blank" class="btn btn-primary font-weight-bolder mr-3">
                                Print
                            </a>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table class="table dt-table-hover" id="zero-config" role="grid"
                                        ...
                                        <tbody>
                                            @if(isset($jobs_cancelled) && !empty($jobs_cancelled) && count($jobs_cancelled))
                                                @foreach($jobs_cancelled as $key => $value)
                                                    @php
                                                        $key = $key + 1;
                                                    @endphp
                                                    <tr>
