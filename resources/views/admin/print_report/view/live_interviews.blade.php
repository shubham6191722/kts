                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        <div class="card-title">
                            <h3 class="card-label">Live interviews</h3>
                        </div>
                        <div class="card-toolbar">
                            <a href="{{route($route_name.'.reportPrintActionPrint',['id' => $user_id.'?action=live_interviews'])}}" target="_blank" class="btn btn-primary font-weight-bolder mr-3">
                                Print
                            </a>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table class="table dt-table-hover" id="zero-config" role="grid"
                                        ...
                                        <tbody>
                                            @if(isset($live_interviews) && !empty($live_interviews) && count($live_interviews))
                                                @foreach($live_interviews as $key => $value)
                                                    @php
                                                        $key = $key + 1;
                                                    @endphp
                                                    <tr>
                                                        ...
                                                        <td>
                                                            @php
                                                                if($value['job_reference'] == 1){
                                                                    $user = App\Models\RecruiterCandidate::recruiterCandidateName($value['r_c_id']);
                                                                }else{
                                                                    $user = App\Models\User::getUserName($value['user_id']);
                                                                }
                                                            @endphp
                                                            {!! $user !!}</td>
                                                        <td>{!! App\Models\JobVacancy::jobName($value['vacancy_id']) !!}</td>
                                                        <td>{!! App\CustomFunction\CustomFunction::get_date_forment($value['event_date']) !!}</td>
                                                        <td>{!! App\CustomFunction\CustomFunction::get_time_forment($value['event_time']) !!}</td>
