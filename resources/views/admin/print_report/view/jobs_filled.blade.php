                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        <div class="card-title">
                            <h3 class="card-label">Jobs filled</h3>
                        </div>
                        <div class="card-toolbar">
                            <a href="{{route($route_name.'.reportPrintActionPrint',['id' => $user_id.'?action=jobs_filled'])}}" target="_blank" class="btn btn-primary font-weight-bolder mr-3">
                                Print
                            </a>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table class="table dt-table-hover" id="zero-config" role="grid"
                                        ...
                                        <tbody>
                                            @if(isset($jobs_filled) && !empty($jobs_filled) && count($jobs_filled))
                                                @foreach($jobs_filled as $key => $value)
                                                    @php
                                                        $key = $key + 1;
                                                    @endphp
                                                    <tr>
