                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        ...
                        <div class="card-toolbar">
                            <a href="{{route('recruiter.recruiterCandidateAdd')}}" class="btn btn-primary font-weight-bolder">
                                    ...
                                    <table class="table dt-table-hover" id="zero-config" role="grid" aria-describedby="kt_datatable_info" style="width: 100%;">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th>Work Base Preferences</th>
                                                <th>CV</th>
                                                <th>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @if(!empty($recruiter_candidate))
                                                @foreach($recruiter_candidate as $key => $value)
                                                    <tr class="odd">
                                                        <td class="dtr-control" tabindex="{!!$key+1!!}">{!!$key+1!!}</td>
                                                        <td>@if(isset($value['job_id']) && !empty($value['job_id'])){!! App\Models\JobVacancy::jobName($value['job_id']) !!}@endif</td>
                                                        <td>@if(isset($value['fname']) && !empty($value['fname'])){!! $value['fname'] !!}@endif @if(isset($value['lname']) && !empty($value['lname'])){!! $value['lname'] !!}@endif</td>
                                                        <td>@if(isset($value['notice_period']) && !empty($value['notice_period'])){!! $value['notice_period'] !!}@endif</td>
                                                        <td>@if(isset($value['salary_expectations']) && !empty($value['salary_expectations']))£{!! App\CustomFunction\CustomFunction::number_format($value['salary_expectations']) !!}@endif</td>
                                                        <td>@if(isset($value['work_base_preferences']) && !empty($value['work_base_preferences'])){!! $value['work_base_preferences'] !!}@endif</td>
                                                        <td>
                                                            @if(isset($value['cv']) && !empty($value['cv']))
                                                                @php
                                                                    $fileUrl = null;
                                                                    if(isset($value['cv']) && !empty($value['cv'])){
                                                                        $file = $value['cv'];
                                                                        $check_ext = explode('.', $file);
                                                                        $fileUrl = url('uploads').'/job_applied/'.$value['job_id'].'/'.$file;
                                                                    }
                                                                @endphp
                                                                <a href="{{$fileUrl}}" class="btn btn-sm btn-light btn-hover-primary btn-icon mr-2 icon-bold" data-theme="dark" data-html="true" title="" data-toggle="tooltip" data-target="#skillEdit_0" data-original-title="Download"> 
                                                                    <span class="svg-icon svg-icon-md">
                                                        ...
                                                        <td nowrap="nowrap">
                                                            <a href="{{route('recruiter.recruiterCandidateEdit',['id' => $value['id']])}}" class="btn btn-sm btn-light btn-hover-primary btn-icon mr-2" data-toggle="tooltip" data-theme="dark" data-html="true" title="Edit"> 
                                                                <span class="svg-icon svg-icon-md"> 
                                                            ...
                                                            <form action="{{ route('recruiter.recruiterCandidateDelete')}}" method="post" >
