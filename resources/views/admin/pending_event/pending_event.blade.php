                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        <div class="card-title">
                            <h3 class="card-label">Interviews List
                            </h3>
                        </div>
                        <div class="card-toolbar">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu p-0 m-0 dropdown-menu-right dropdown-menu-anim-up dropdown-menu-lg">
                                    ...
                                    <form method="GET" action="{{route($route_name.'.pendingEventGet')}}" enctype="multipart/form-data">
                                        <div class="tab-content">
                                            <div class="tab-pane active show px-4">
                                                <div>
                                                    ...
                                                    @endif
                                                    <div class="form-group mb-3">
                                                        <label class="fs-6 fw-semibold mb-2">Select Vacancy</label>
                                                        <div>
                                                            <select class="form-control select2" name="user_vacancy" id="user_vacancy">
                                                                <option selected value="all">All</option>
                                                                @if(!empty($vacancy_data))
                                                                    @foreach($vacancy_data as $VDKey => $vacancy_value)
                                                                        <option value="{!! $vacancy_value->id !!}" {!! (isset($_GET['user_vacancy']) && $_GET['user_vacancy'] == $vacancy_value->id) ? 'selected' : ''; !!}>{!! $vacancy_value->jobtitle !!}</option>
                                                                    @endforeach
                                                                @endif
                                                            </select>
                                                        </div>
                                                    ...
                                                    <div class="form-group mb-3 text-center">
                                                        <a href="{{route($route_name.'.pendingEventGet')}}" class="btn btn-light me-3">Reset</a>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            @if($value['event_status'] == 0)
                                                                <span class="label label-lg label-light-warning label-inline lable-custom-w-h" data-toggle="tooltip" data-theme="dark" data-html="true"  data-original-title="" title="">Pending</span>
                                                            @elseif($value['event_status'] == 1)
                                                                <span class="label label-lg label-light-success label-inline lable-custom-w-h" data-toggle="tooltip" data-theme="dark" data-html="true"  data-original-title="" title="">Confirmed</span>
                                                            @elseif($value['event_status'] == 2)
                                                                <span class="label label-lg label-light-danger label-inline lable-custom-w-h" data-toggle="tooltip" data-theme="dark" data-html="true"  data-original-title="" title="">Rejected</span>
                                                            @elseif($value['event_status'] == 3)
                                                                <span class="label label-lg label-light-dark label-inline lable-custom-w-h" data-toggle="tooltip" data-theme="dark" data-html="true"  data-original-title="" title="">Canceled</span>
                                                            @endif
                                                            @if(Auth::user()->role == 1)
                                                                @if($value['check_time_slot'] == 1)
                                                                    <span class="label label-lg label-light-warning label-inline lable-custom-w-h" data-toggle="tooltip" data-theme="dark" data-html="true"  data-original-title="" title="">Normal</span>
                                                                @else
                                                                    <span class="label label-lg label-light-dark label-inline lable-custom-w-h" data-toggle="tooltip" data-theme="dark" data-html="true"  data-original-title="" title="">Outlook</span>
                                                                @endif
                                                            @endif
                                                        </td>
                                                        <td  nowrap="nowrap">
                                ...
                                <div class="fv-row mb-9">
                                    <div class="form-group">
                                        ...
                                        <div class="event-view-select-box">
                                            @php
                                                $user_client = App\Models\User::clientDataData($r_value->client_id);
                                                $user_staff = App\Models\User::staffList($r_value->client_id);
                                                $user_clent = App\Models\User::clientListForClient($r_value->client_id);
                                                $staff_data = $user_staff->concat($user_clent)->shuffle();
                                                $staff = $staff_data->concat($user_client)->shuffle();
                                                $event_staff_data = array();
            ...
            <script>
                jQuery(document).ready(function() {
                    $('body').on('change', '#user_select', function() {
                        ...
                        getUserVacancy(user_id);
                    });
                });
                function getUserVacancy(user_id) {
                    $.ajaxSetup({
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        }
                    ...
                    $.ajax({
                        url: "{!! route('comman.getUserVacancy') !!}",
                        dataType: 'json',
                        type: 'post',
                        data: {
                            id: user_id,
                        ...
                        beforeSend: function() {
                            // $.LoadingOverlay("show");
                            $('#user_vacancy').empty().select2();
                        },
                        success: function(data) {
                            if (data.code == 1) {
                                if (data.vacancy_data != '') {
                                    var option = '';
                                    var html = '<option label="Label" value="all" selected disabled>All</option>';
                                    $(data.vacancy_data).each(function(index, item) {
                                        html += '<option value="' + item.id + '" >' + item.jobtitle +'</option>';
                                    });
                                    $('#user_vacancy').append(html).trigger('change');

                                    $('#user_vacancy').trigger({
                                        type: 'select2:select',
                                        params: {
                                            data: data
                                        }
                                    ...
                                    var html = '';
                                    html += '<option label="Label" value="all">All</option>';
                                    $('#user_vacancy').append(html).trigger('change');
                        ...
                        error: function(jqXhr, textStatus,errorThrown) {
                            // $.LoadingOverlay("hide");
                            show_toastr(textStatus,'Please reload & try again','');
                        }
                    });
                }
            </script>
