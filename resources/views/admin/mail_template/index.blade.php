                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        <div class="card-title">
                            <h3 class="card-label">Mail Template</h3>
                        </div>
                        <div class="card-toolbar">
                            {{-- @if(Auth::user()->role == 1)

                            @else --}}
                                <a href="{{route($route_name.'.emailTemplateAdd')}}" class="btn btn-primary font-weight-bolder">
                                    ...
                                    <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer" id="zero-config" role="grid"
                                        ...
                                        <tbody class="fw-semibold text-gray-600">
                                            @if(!empty($mail_template))
                                                @foreach($mail_template as $key => $value)
                                                    <tr class="odd">
                                                        <td class="dtr-control" tabindex="{!!$key+1!!}">{!!$key+1!!}</td>
                                                        <td>@if(isset($value->template_title) && !empty($value->template_title)){!! $value->template_title !!}@endif</td>
                                                        @if(Auth::user()->role == 1)
                                                        @else
                                                            @if($check_sub_company)
                                                                <td>@if(isset($value->sub_company) && !empty($value->sub_company)){!! App\Models\SubCompany::getSubCompanyName($value->sub_company) !!}@else -- @endif</td>
                                                            @endif
                                                        @endif
                                                        <td>
                                                            @php
                                                        ...
                                                        <td nowrap="nowrap">
                                                            <a href="{{route($route_name.'.emailTemplateEdit',['id' => $value['id']])}}" class="btn btn-sm btn-light btn-hover-primary btn-icon mr-2" data-theme="dark" data-html="true" title="Edit" data-toggle="tooltip" data-target="#skillEdit_{{$key}}"> 
                                                                <span class="svg-icon svg-icon-md">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                                        <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                                            <rect x="0" y="0" width="24" height="24"></rect>
                                                            ...
                                                            <form action="{{ route($route_name.'.emailTemplateDelete')}}" method="post" >
