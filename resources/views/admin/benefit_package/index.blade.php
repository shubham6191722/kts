...
@section('title', 'Company Benefits Workflows')
                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap py-5">
                        <div class="card-title">
                            <h3 class="card-label">Company Benefits Workflows</h3>
                        </div>
                        <div class="card-toolbar">
                            <a href="{{route('client.benefitPackageAdd')}}" class="btn btn-primary font-weight-bolder">
                                ...
                                </span>Add Company Benefits Workflows</a>
                        </div>
                    </div>
                    <div class="card-body">
                                    ...
                                    <table class="table dt-table-hover" id="zero-config" role="grid"
                                        ...
                                        <thead>
                                            <tr role="row">
                                                <th>Sr No.</th>
                                                <th>Title</th>
                                                <th>Company Benefits Workflows detail</th>
                                                <th>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @if(!empty($benefit_package))
                                            @foreach($benefit_package as $key => $value)
                                            <tr class="odd">
                                                <td class="dtr-control" tabindex="{!!$key+1!!}">{!!$key+1!!}</td>
                                                <td>@if(isset($value->title) && !empty($value->title)){!! $value->title !!}@endif</td>
                                                <td>@if(isset($value->description) && !empty($value->description)){!! $value->description !!}@endif</td>
                                                <td nowrap="nowrap">
                                                    <a href="{{route('client.benefitPackageEdit',['id' => $value->id])}}" class="btn btn-sm btn-light btn-hover-primary btn-icon mr-2" data-theme="dark" data-html="true" title="Edit"> 
                                                        <span class="svg-icon svg-icon-md"> 
                                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                                    <rect x="0" y="0" width="24" height="24"></rect>
                                                    ...
                                                    <form action="{{ route('client.benefitPackageDelete')}}" method="post" >
