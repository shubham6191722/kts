...
class OfflineCandidateJobs implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($client_id,$candidate_id,$message_data,$client_company_name,$job_name,$candidate_name = null,$recruiter_name = null,$message_name = null,$job_id=null)
    {
    ...
    public function handle()
    {
        ...
        if(isset($company_full_data->company_logo) && !empty($company_full_data->company_logo)){
            ...
            if (isset($siteSetting) && !empty($siteSetting)) {
                $company_logo = url('uploads') . '/site_setting/'.$siteSetting->site_header_logo;
            }
        }

        $email = User::getEmail($candidate_id);
        Log::info("User id:- ".$candidate_id);
        Log::info("User chat normal Email:- ".$email);
