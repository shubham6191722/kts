...
class RecruiterMailNotificationInQueue implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($user_id, $job_id, $email,$notifications_type,$candidate_id = null,$event_type = null,$r_c_id = null,$link = null)
    {
    ...
    public function handle()
    {
        ...
        if ((isset($user_id) && !empty($user_id)) && (isset($job_id) && !empty($job_id)) && (isset($email) && !empty($email))) {
            ...
            $recruiter_data = User::find($r_c_id);
            $job_vacancy = JobVacancy::find($job_id);
            $candidate_data = RecruiterCandidate::recruiterCandidateData($candidate_id);
            ...
            if(isset($recruiter_data) && !empty($recruiter_data)){
                ...
                if(isset($recruiter_data->name) && !empty($recruiter_data->name)){
                    $recruiter_name = $recruiter_data->name;
                }

                if(isset($recruiter_data->lname) && !empty($recruiter_data->lname)){
                    $recruiter_name = $recruiter_name." ".$recruiter_data->lname;
                }
                
                if(isset($recruiter_data->company_name) && !empty($recruiter_data->company_name)){
                    $recruiter_company = $recruiter_data->company_name;
                }
            }
            ...
            if(isset($candidate_data) && !empty($candidate_data)){
                ...
                if(isset($candidate_data->fname) && !empty($candidate_data->fname)){
                    $candidate_name = $candidate_data->fname;
                }
            ...
            $candidate_name = $candidate_name.'('.$recruiter_company.')';
            ...
            $emailDescription = str_replace("{recruiter-name}", $recruiter_name, $emailDescription);
            ...
            $trimedSubject = str_replace("{recruiter-name}", $recruiter_name, $trimedSubject);
            $trimedSubject = str_replace("{candidate-name}", $candidate_name, $trimedSubject);
            $trimedSubject = str_replace("{vacancy-title}", $candidate_name, $trimedSubject);
            $trimedSubject = str_replace("{company-name}", $candidate_name, $trimedSubject);
            $trimedSubject = str_replace("{event-type}", $candidate_name, $trimedSubject);
